...
export default function EventTile({ event }) {
	...
	if (startDateObj && endDateObj) {
		...
		if (sameYear && sameMonth && sameDay) {
			...
			const startMonthDay = startDateObj.toLocaleDateString(
				undefined,
				optsMonthDay
			);
			const endMonthDay = endDateObj.toLocaleDateString(
				undefined,
				optsMonthDay
			);
	...
	return (
		<div className="aspect-[360/575] flex flex-col w-full rounded-lg overflow-hidden bg-white shadow-md">
			...
			{/* Text will expend as needed base on text size */}
			...
			{/* Button now is fixed/padded area */}
			<div className="h-[15%] flex justify-center items-center">
				<a href={detailsurl}>
					<button className=" hover:bg-[#bf5d7e] px-10 py-2 bg-[#D96B91] text-white rounded-full text-md cursor-pointer transform hover:scale-105 transition-transform duration-200 ease-in-out">
						Details
					</button>
				</a>
			</div>
